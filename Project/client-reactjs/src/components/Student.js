...
const Student = () => {
  ...
  localStorage.setItem("tickettype", "student");
  ...
  const addRecord = () => {
    ...
    if (
      recordCount === 1 &&
      (!phone || !email || !name || !age || !reg_no || !college_name)
    ) {
      setShowAlert(true);
    ...
    if (recordCount > 1 && (!name || !age || !reg_no || !college_name)) {
      setShowAlert(true);
    ...
    setRecords([
      ...records,
      { id: recordCount, name, age, reg_no, college_name },
    ]);
    setRecords1([...records1, { name, age, date, type }]);
    setRecords2([...records2, { name, reg_no, college_name }]);
  ...
  const removeRecord2 = (name) => {
    // Remove record from the state
    const updatedRecords = records2.filter((record) => record.name !== name);
  ...
  const handleSubmit = () => {
    ...
    localStorage.setItem("records2", JSON.stringify(records2));
      ...
      >
        ...
        {showAlert && !reg_no && (
          <div
            style={{ color: "red", marginTop: "-25px", marginBottom: "5px" }}
          >
            Please enter a Registeration no.!
          </div>
        )}
